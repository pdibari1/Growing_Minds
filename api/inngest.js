  ...
  async ({ event, step }) => {
    ...
    if (process.env.OPENAI_API_KEY && process.env.SKIP_ILLUSTRATIONS !== "true") {
      ...
      const finalCast = await step.run("generate-cover-and-character-sheet", async () => {
        const coverChap = freshOutline[0];
        const { friend } = childData;
        const friendLine = friend && friend !== "none" ? ` Their companion ${friend} is also present.` : "";
        const mainCharDesc = castDescriptions[name] || `a ${age}-year-old ${genderDesc} with ${hairDesc} hair and ${eye} eyes`;
        const coverPrompt = `${styleGuide}. ${coverChap.imagePrompt} The main character ${name} is ${mainCharDesc}.${friendLine} Setting: ${city}, ${region}. All characters are young children with no facial hair. Cheerful, warm, child-friendly illustration. No text or words in the image.`;
      ...
      for (let b = 0; b < imgBatchesRemaining; b++) {
        await step.run(`generate-illustrations-${b + 1}`, async () => {
          ...
          for (const key of keys) {
            ...
            const prompt = `${styleGuide}. ${chap.imagePrompt} Setting: ${city}, ${region}. The main character ${name} is ${mainCharDesc}. Draw an active story scene — not a character lineup or portrait. All characters are young children with no facial hair. Cheerful, child-friendly. No text or words in the image.`;
            try {
              let imageUrl;
              try {
                ...
                const fallbackPrompt = `${styleGuide}. ${name} exploring outdoors in ${city}, ${region} on a sunny day. ${name} is ${castDescriptions[name] || `a ${age}-year-old ${genderDesc} with ${hairDesc} hair`}. All characters are young children with no facial hair. Cheerful, child-friendly. No text or words in the image.`;

        ...
        html: `
          <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1a1a2e;">
            ...
            <div style="background:#fefae0;padding:2rem;border-radius:0 0 12px 12px;border:1px solid #e5e7eb;">
              ...
              <p style="margin-top:1rem;color:#6b7280;font-size:.9rem;">Ready for the full 30-chapter story? Order the complete softcover book and it will be printed and shipped to your door.</p>
...
EARLY PACING RULE: In the chapters immediately following the ${genre || 'fantasy'} hook (roughly Chapters 2–6), do not compress a whole sequence of escalating attempts, tests, or feats into a single chapter — a chapter where the hero tries the new power/adventure element five different ways and succeeds at each, arriving at a fully confirmed rule, advances several chapters' worth of plot in the space of one. Give each significant attempt its own room: one attempt, one partial or inconsistent result, then stop — sit with the not-knowing rather than chaining several successful escalating attempts back to back in the same chapter. Discoveries should also raise at least one new question before fully answering an old one — early results should be unreliable or incomplete rather than immediately yielding a clear, confirmed rule. The hook must still ignite and be clearly underway by Chapter 3 (per above), but "underway" means the hero is investigating and forming theories across multiple chapters, not that they've already run the full experiment and arrived at a fully solved rule by Chapter 2 or 3.
...
PRACTICE-THEN-SUCCEED RULE: Before the climax, ${name} must genuinely attempt the same kind of response that eventually succeeds — the actual coping response, skill, or approach relevant to the milestone — at least once, in a lower-stakes moment, and have it fall short or only partly work. Don't skip straight from "the problem happens" to "the hero fully succeeds" — show them trying, falling short, feeling that disappointment, and only later, when it matters most, doing a fuller or more practiced version of that same response and having it work. This is distinct from an external setback (see MILESTONE CAUSALITY RULE) — it's specifically ${name} trying the real thing and it not fully working yet, not just something going wrong around them.

INTERPERSONAL MISTAKE RULE: Somewhere in the story, ${name}'s struggle with the milestone must cause them to treat another named character poorly — snapping at them, dismissing their help or idea, blaming them unfairly, or something similarly real — not just an internal setback for ${name} alone. ${name} must later notice this, recognize they were wrong, and actively repair it (an apology, an action that makes it right, a real acknowledgment) rather than the other character simply forgiving it without ${name} addressing it. This gives the milestone a second dimension: it's not just about ${name} feeling better, it's about what ${name}'s struggle costs the people around them.

TEAM CONTRIBUTION RULE: If the story includes named peer allies (friends, siblings, a team ${name} has built), at least one or two of them must make a specific, necessary contribution during the climax itself — not just cheer or celebrate afterward. Give each contributing ally something concrete tied to their established role or skill (they notice something crucial, solve a piece of the problem, provide something needed, keep ${name} steady in the moment) that the climax genuinely needs. ${name} should still be the one who takes the final decisive action (see ADULT ROLE RULE and IDENTITY RULE) — the goal is a hero who isn't the ONLY hero, not a hero who doesn't act.

PEER DEPTH RULE: If the story includes named peer allies, give each one at least one trait, moment, or detail that belongs to them as an individual — not just their functional role in the group (the analytical one, the energetic one, etc.). This could be a fear unrelated to the plot, a running joke, a disagreement with ${name} about something, or a vulnerability of their own — something that would still be true of them even if their job in the story were different.
...
TRUST THE METAPHOR RULE: Once this story's central fantasy mechanism (how the magic/power actually works) has been clearly established and demonstrated once, do not have characters, narration, or written notes re-explain that same underlying rule again in later chapters. Show ${name} applying, encountering, or struggling with it through action and consequence instead — trust the reader to recognize the mechanism without it being restated in words. Only revisit an explicit explanation if the story is deliberately revealing a genuinely NEW layer of understanding, not restating what's already established.
...
BASELINE RULE: Before the ${genre || 'fantasy'} hook ignites in Chapters 1–2, include one small, ordinary, non-magical moment showing ${name}'s typical reaction to the milestone trigger — something low-stakes and real (a correction from a parent, a suggestion from a friend, a small interruption) that shows their default response pattern, without resolving or transforming it yet. This gives the story a concrete "before" to measure growth against, rather than the reader's first exposure to the trigger-response pattern happening only inside the magical mechanism.
...
- SETTING DETAIL: When describing a real-world place ${name} spends time in (classroom, gym, cafeteria, etc.), keep it general and atmospheric — never invent specific physical layout details (how many desks or tables, their arrangement, room shape, decorations) that could differ from the real place. Say "his classroom" or "the noisy classroom," not "six tables arranged in a horseshoe."
- SCENE CONTINUITY: Open each chapter by picking up the "Carries forward" thread listed for it above — the same unresolved problem, goal, question, or emotion the previous chapter left off on. Don't open with an unexplained new setting or a "the next day at ___" reset with nothing connecting it to what just happened; if the location changed, a sentence or two should make clear why it changed now, driven by what just happened, not just because time passed. When a chapter returns to a previously-established setting (home, the basement, the kitchen, etc.), ground the reader with one brief orienting phrase near the start of the scene — a familiar, already-established detail (the kitchen light, a pet, the stairs) works well — before moving into the chapter's actual action or dialogue.
- TRUST THE METAPHOR: Once the story's central mechanism has been clearly explained once in an earlier chapter, don't have this chapter's characters or narration re-explain the same underlying rule in words again — show it through action and consequence instead. Only add new explanation if it reveals a genuinely new layer of understanding beyond what's already established.
- VARY THE TRIGGER RESPONSE: Each time ${name} experiences the milestone trigger, vary the specific physical or behavioral manifestation instead of repeating the identical cue checklist every time (e.g., don't always describe heat rising, jaw tightening, and feet grounding, in that order, every single time). Draw from a range of real reactions — talking faster, going quiet and not hearing what's said, hands going rigid, wanting to leave the room, replaying what he wishes he'd said, avoiding eye contact, and so on — so each instance feels like a distinct real moment, not a repeating mechanism. The underlying pattern (trigger, dysregulation, reconnecting, responding differently) can stay consistent even as the specific details vary.

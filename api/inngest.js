...
function getStyleGuideForAge(age) {
  ...
  if (ageNum <= 7) {
    return "Disney animated feature film style. A fully painted digital illustration — rich, luminous, cinematic. Large expressive eyes with detailed irises and bright catchlights. Soft rounded cartoon features with warm glowing skin. Warm amber-gold color temperature with vibrant saturated colors. Dramatically voluminous flowing hair with rich color, fine strand detail, and warm golden rim lighting creating a glowing halo effect. Lush storybook outdoor background filled edge to edge — warm landscapes, colorful wildflowers, trees in golden light. The character radiates warmth and joy. Painterly detail throughout — no flat fills, no outlines, no coloring-book look. Pure Disney/Pixar quality digital art.";
  } else if (ageNum <= 11) {
...
function getCoverStyleForAge(age) {
  const ageNum = parseInt(age) || 7;
  if (ageNum <= 7) {
    return "Disney animated feature film style. A breathtaking close-up portrait illustration — fully painted, cinematic quality. The character fills the frame from the chest up, smiling warmly with enormous expressive eyes with rich detailed irises and bright catchlights. Dramatically voluminous flowing hair with warm amber-gold backlighting creating a glowing halo effect — rich fine strand detail throughout. Soft luminous skin with warm glowing shading. Warm amber-gold color temperature throughout with vibrant saturated colors. Lush storybook outdoor background filling the frame edge to edge — warm landscapes, colorful wildflowers, trees bathed in golden light. Painterly digital art with rich texture and depth — no flat fills, no outlines, no coloring-book style. Pure Disney/Pixar quality digital illustration.";
  } else if (ageNum <= 11) {

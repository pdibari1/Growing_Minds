// api/regenerate.js — Admin clicks "Regenerate" in the notification email
...
module.exports = async function handler(req, res) {
  ...
  try {
    // Tell the waiting Inngest run to skip delivery when it wakes up
    await redisRequest("SET", [`skip-delivery:${storyId}`, "1", "EX", 86400]);

    // Retrieve the original story token from Redis so we can re-fire generation
    ...
    if (!storyToken) {
      return res.status(404).send(`
        <html><body style="font-family:sans-serif;max-width:480px;margin:80px auto;text-align:center;color:#1a1a2e;">
          ...
          <p style="color:#6b7280;">The story token is no longer in Redis (it may have been cleaned up). Re-fire manually from the Inngest dashboard using story ID: <code>${storyId}</code></p>
        </body></html>
      `);
    }

    // Fire a brand-new generation run with the same inputs
    ...
    return res.status(200).send(`
      <html><body style="font-family:sans-serif;max-width:480px;margin:80px auto;text-align:center;color:#1a1a2e;">
        ...
        <h2 style="color:#2d6a4f;">Regeneration started!</h2>
        <p style="color:#6b7280;">A new story is being generated for ${childName || 'this customer'}. You'll receive a new review email when it's ready.</p>
        <p style="color:#9ca3af;font-size:.85rem;margin-top:1rem;">The original will not be sent.</p>
      </body></html>
    `);
  } catch (err) {
    console.error("Regenerate error:", err.message);
    return res.status(500).send("Failed to regenerate — check Inngest logs");
  }
};
